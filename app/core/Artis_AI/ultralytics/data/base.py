...
class BaseDataset(Dataset):
    ...
    def load_rgbd_data(self, i: int, rect_mode: bool = True) -> tuple[np.ndarray, tuple[int, int], tuple[int, int]]:
        """
        Make rgb-d (4ch) data for rgb-d model after Load an image and depth from dataset index 'i'

        Args:
            i (int): Index of the image & depth data to load.
            rect_mode (bool): Whether to use rectangular resizing.

        Returns:
            im (np.ndarray): Loaded rgb-d data as a NumPy array.
            hw_original (tuple[int, int]): Original image dimensions in (height, width) format.
            hw_resized (tuple[int, int]): Resized image dimensions in (height, width) format.
        ...
        filename_depth = f.replace(f_ext, ".bin")
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                try:
                    im = np.load(fn)
                    ...
                    if os.path.exists(filename_depth):
                        try:
                            with open(filename_depth, "rb") as bin_depth_file:
                                len_depth_header = np.fromfile(bin_depth_file, dtype=np.uint16, count=1)[0]
                                category_depth_header = bin_depth_file.read(len_depth_header).decode('utf-8')
                            ...
                            raise FileNotFoundError(f"Invalid Depth file, Please Check : {filename_depth}")

                    else:
                        raise FileNotFoundError(f"Depth file Not Found {filename_depth}")
            else:  # read image
                im = imread(f, flags=self.cv2_flag)  # BGR
                with open(filename_depth, "rb") as bin_depth_file:
                    len_depth_header = np.fromfile(bin_depth_file, dtype=np.uint16, count=1)[0]
            ...
            if depth is None:
                raise FileNotFoundError(f"Depth file Not Found {filename_depth}")
            resize_depth = cv2.resize(depth, dsize=(1280, 960), interpolation=cv2.INTER_LINEAR)
            resize_depth = np.expand_dims(resize_depth, axis=-1)

            rgbd_data = np.concatenate((im, resize_depth), axis=-1)
            im = rgbd_data.copy()
    ...
    def get_image_and_label(self, index: int) -> dict[str, Any]:
        ...
        ##### Load RGB+Depth Data #####
            ...
            label["img"], label["ori_shape"], label["resized_shape"] = self.load_rgbd_data(index)
        label["ratio_pad"] = (
