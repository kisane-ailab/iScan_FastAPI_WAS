...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic3(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        for i in range(3):
            ...
            if i == 0:  # center
                if img.shape[-1] == 3:
                    img3 = np.full((s * 3, s * 3, img.shape[-1]), 114, dtype=np.uint8)  # base image with 3 tiles
                else:
                    img3 = np.zeros((s * 3, s * 3, img.shape[-1]), dtype=np.uint16)
                    img3[:] = np.array([114, 114, 114, 470], dtype=np.uint16)
                h0, w0 = h, w
    ...
    def _mosaic4(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        for i in range(4):
            ...
            if i == 0:  # top left
                if img.shape[-1] == 3:
                    img4 = np.full((s * 2, s * 2, img.shape[-1]), 114, dtype=np.uint8)  # base image with 4 tiles
                else:
                    img4 = np.zeros((s * 2, s * 2, img.shape[-1]), dtype=np.uint16)
                    img4[:] = np.array([114, 114, 114, 470], dtype=np.uint16)
    ...
    def _mosaic9(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        for i in range(9):
            ...
            if i == 0:  # center
                if img.shape[-1] == 3:
                    img9 = np.full((s * 3, s * 3, img.shape[-1]), 114, dtype=np.uint8)  # base image with 4 tiles
                else:
                    img9 = np.zeros((s * 3, s * 3, img.shape[-1]), dtype=np.uint16)
                    img9[:] = np.array([114, 114, 114, 470], dtype=np.uint16)
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        #Mix-up Augmentation only can do in rgb images, rgb-d data is not possible.
        if labels["img"].shape[-1] == 3:
...
class RandomPerspective:
    ...
    def affine_transform(self, img: np.ndarray, border: tuple[int, int]) -> tuple[np.ndarray, np.ndarray, float]:
        ...
        # Affine image
        cur_data_channel = img.shape[2]
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                ##### RGB Data #####
                if cur_data_channel == 3:
                    img = cv2.warpPerspective(img, M, dsize=self.size, borderValue=(114, 114, 114))
                ##### RGB-D Data #####
                else:
                    img = cv2.warpPerspective(img, M, dsize=self.size, borderValue=(114, 114, 114, 470))
            else:  # affine
                ##### RGB Data #####
                if cur_data_channel == 3:
                ...
                ##### RGB-D Data #####
                    ...
                    img = cv2.warpAffine(img, M[:2], dsize=self.size, borderValue=(114, 114, 114, 470))
            if img.ndim == 2:
                img = img[..., None]
        return img, M, s
...
class RandomHSV:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            if img.shape[-1] == 4:
                img = np.concatenate((rgb_img, depth), axis=-1)
            else:
                img = rgb_img.copy()
        labels["img"] = img
        return labels
...
class LetterBox:
    ...
    def __call__(self, labels: dict[str, Any] = None, image: np.ndarray = None) -> dict[str, Any] | np.ndarray:
        ...
        if c == 3:
            ...
            padding_value_for_rgbd = np.array([114, 114, 114, 470], dtype=img.dtype).reshape(1, 1, c)
            pad_img = np.tile(padding_value_for_rgbd, (h + top + bottom, w + left + right, 1))
            pad_img[top : top + h, left :  left + w] = img
            img = pad_img
...
class Albumentations:
    ...
    def __init__(self, p: float = 1.0) -> None:
        ...
        try:
            ...
            T = [
                ...
                A.ColorJitter(p=0.3, brightness=(0.9, 1.1), contrast=(0.9, 1.1), saturation=(0.9, 1.1), hue=(-0.1, 0.1)),
            ]
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        #    return labels
        if self.contains_spatial and im.shape[2] > 3:
            print(f"Spatial Augmentation by using Albumentation is not support with RGB-D data.")
            return labels

        ##### Only works with rgb data #####
        if self.contains_spatial:
            cls = labels["cls"]
            if len(cls):
        ...
        ##### both rgb and rgb-d data supported #####
        else:
            ##### if rgb #####
            if im.shape[2] == 3:
            ...
            ##### if rgbd #####
                ...
                data_rgb, data_depth = im[:, :, :3].astype(np.uint8), im[:, :, 3:4].astype(np.uint16)
                data_rgb = self.transform(image=data_rgb)["image"]
                labels["img"] = np.concatenate((data_rgb, data_depth), axis=-1)
...
class Format:
    ...
    def _format_img(self, img: np.ndarray) -> torch.Tensor:
        ...
        """
        print(f"[Format] Entered format_img")
        if len(img.shape) < 3:
        ...
        ##### RGB-D Data #####
            ...
            rgb_img = img[:3, :, :]
            depth = img[3:4, :, :]
            rgb_img = np.ascontiguousarray(rgb_img[::-1] if random.uniform(0, 1) > self.bgr else rgb_img)
            img = np.concatenate((rgb_img, depth), axis=0)
