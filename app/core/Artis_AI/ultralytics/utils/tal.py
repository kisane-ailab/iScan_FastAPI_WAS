...
class TaskAlignedAssignerObject(TaskAlignedAssigner):
    def _forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        """
        Compute the task-aligned assignment. Reference code is available at
        https://github.com/Nioolek/PPYOLOE_pytorch/blob/master/ppyoloe/assigner/tal_assigner.py.
        ...
        # Assigned target
        target_labels, target_bboxes, target_scores = self.get_targets(gt_labels, gt_bboxes, target_gt_idx, fg_mask)
        original_shape_tensor = target_scores.clone()  # 텐서 복사
        objectness_idx = target_scores.shape[-1] - 1
    
        flat_mask = fg_mask.bool().view(-1)
        target_scores = target_scores.view(-1, target_scores.shape[-1])
        target_scores[flat_mask, objectness_idx] = 1.0
        target_scores = target_scores.view_as(original_shape_tensor)
        # Normalize
        align_metric *= mask_pos
        pos_align_metrics = align_metric.amax(dim=-1, keepdim=True)  # b, max_num_obj
        pos_overlaps = (overlaps * mask_pos).amax(dim=-1, keepdim=True)  # b, max_num_obj
