...
from ultralytics.utils.tal import RotatedTaskAlignedAssigner, TaskAlignedAssigner, TaskAlignedAssignerObject, dist2bbox, dist2rbox, make_anchors
...
class v8DetectionLoss_Object(v8DetectionLoss):
    
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        """Initializes the v8DetectionLoss_Object class, taking a de-paralleled model as argument."""
        super().__init__(model)
        self.assigner = TaskAlignedAssignerObject(topk=tal_topk, num_classes=self.nc, alpha=0.5, beta=6.0)
    def __call__(self, preds, batch):
        """Calculate the sum of the loss for box, cls and dfl multiplied by batch size."""
        loss = torch.zeros(3, device=self.device)  # box, cls, dfl
        feats = preds[1] if isinstance(preds, tuple) else preds
