...
def get_box_iou_from_boxes_torch(boxes):
    """
    Params:
        boxes : (torch.tensor) (N, 4) [tlx, tly, brx, bry]
    Returns:
        iou : (torch.tensor) (N, N) iou matrix
    """
    N = boxes.size(0)
    ...
    # broadcasting (N, N)
    inter_x1 = torch.max(x1, x1.T)
    inter_y1 = torch.max(y1, y1.T)
    inter_x2 = torch.min(x2, x2.T)
    inter_y2 = torch.min(y2, y2.T)
...
def fast_nms(boxes, scores, iou_threshold=0.3):
    """
    Params:
        boxes : (torch.tensor) (N, 4) [tlx, tly, brx, bry]
        scores : (torch.tensor) (N, ) class scores
    ...
    # iou matrix
    iou_matrix = get_box_iou_from_boxes_torch(boxes)

    # use triangular matrix
    iou_triu = torch.triu(iou_matrix, diagonal=1)

    # suppress
    suppressed = (iou_triu > iou_threshold).any(dim=0)

    # keep
    keep = torch.nonzero(~suppressed).squeeze(1)
