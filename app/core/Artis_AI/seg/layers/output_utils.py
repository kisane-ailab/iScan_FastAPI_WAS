...
#from seg.data import (cfg, mask_type, activation_func,
#                      SEG_MEANS_1CH, SEG_STDS_1CH, SEG_MEANS_3CH, SEG_STDS_3CH, SEG_MEANS_4CH, SEG_STDS_4CH)
from seg.data import cfg, mask_type, MEANS, STD, activation_func
from seg.utils.augmentations import Resize
from seg.utils import timer
from .box_utils import crop, sanitize_coordinates
from .box_utils import fast_nms


def remove_small_object_from_pixel_area(masks, pixel_area_threshold = 70*100):
    """
    Params:
        masks : (Torch) (num_det, h, w)
        pixel_area_threshold : (int)
    Returns:
        keep : (Torch) (N, )
    """
    pixel_area = masks.sum(dim=(1, 2))
    exists_indices = pixel_area > pixel_area_threshold
    keep = torch.nonzero(exists_indices).squeeze(1)

    return keep


def postprocess(det_output, w, h, batch_idx=0, interpolation_mode='bilinear',
                visualize_lincomb=False, crop_masks=True, score_threshold=0, iou_threshold=0.3, pixel_area_threshold = 70*100):
    """
    ...
    # apply fast nms using torch (by MSP)
    idx_arr = fast_nms(boxes, scores, iou_threshold)
    classes = classes[idx_arr]
    boxes   = boxes[idx_arr]
    scores  = scores[idx_arr]
    masks   = masks[idx_arr]
    ...
    # MSP : remove small object
    idx_arr = remove_small_object_from_pixel_area(masks, pixel_area_threshold)
    classes = classes[idx_arr]
    boxes   = boxes[idx_arr]
    scores  = scores[idx_arr]
    masks   = masks[idx_arr]
...
def undo_image_transformation(img, w, h):
    ...
    """
    #Set Default Value To Var MEANS & STD
    #MEANS = SEG_MEANS_3CH
    #STD = SEG_STDS_3CH
