...
#from seg.data import cfg, SEG_MEANS_3CH, SEG_STDS_3CH, SEG_MEANS_4CH, SEG_STDS_4CH, SEG_MEANS_1CH, SEG_STDS_1CH
from seg.data import cfg, MEANS, STD
...
class Pad(object):
    ...
    """
    #def __init__(self, width, height, mean=SEG_MEANS_3CH, pad_gt=True):
...
class BackboneTransform(object):
    ...
    """
    def __init__(self, transform, mean, std, in_channel_order, data_type):
...
class BaseTransform(object):
    ...
    #def __init__(self, mean=SEG_MEANS_3CH, std=SEG_STDS_3CH, data_type='rgb'):
    def __init__(self, mean=MEANS, std=STD, data_type='rgb'):
        self.augment = Compose([
            ConvertFromInts(),
            Resize(resize_gt=False),
            BackboneTransform(cfg.backbone.transform, mean, std, 'BGR', data_type)
        ])
...
class FastBaseTransform(torch.nn.Module):
    ...
    #def __init__(self):
    #def __init__(self, means=SEG_MEANS_3CH, stds=SEG_STDS_3CH):     # add 'MEANS', 'STD' arguments for depth and rgbd
    def __init__(self, MEANS, STD):     # add 'MEANS', 'STD' arguments for depth and rgbd
        super().__init__()

        #self.mean = torch.Tensor(means).float().cuda()[None, :, None, None]
        self.mean = torch.Tensor(MEANS).float().cuda()[None, :, None, None]
        #self.std  = torch.Tensor(stds).float().cuda()[None, :, None, None]
...
class SSDAugmentation(object):
    ...
    #def __init__(self, mean=SEG_MEANS_3CH, std=SEG_STDS_3CH):
