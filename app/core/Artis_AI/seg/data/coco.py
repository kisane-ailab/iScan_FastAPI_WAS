...
class COCODetection(data.Dataset):
    ...
    def __init__(self, image_path, info_file, transform=None,
        ...
        #print(f"=====> [COCODetection] image_path = {image_path}")
        #print(f"=====> [COCODetection] info_file = {info_file}")
    ...
    def pull_item(self, index):
        ...
        # MSP : depth
        if self.data_type in ['depth', 'rgbd']:
            depth_path = path.replace('.jpg', '.bin')
            #depth_filename = filename.replace('_Color.jpg', '_Depth.bin')
            with open(depth_path, 'rb') as bin_depth_file:
        ...
        if self.data_type == 'depth':                           # MSP : use only depth channel
            img = resize_depth                                  # (img_h, img_w, 1)
        elif self.data_type == 'rgbd':
            img = np.concatenate((img, resize_depth), axis=2)   # (img_h, img_w, 4)
    ...
    def pull_item_including_depth(self, index):
        """
        Args:
            index (int): Index
        Returns:
...
if __name__ == "__main__":
    ...
    image_path = "/code/data/coco/images/"
    info_file = "/code/data/coco/annotations/instances_train2017.json"


    #=====> [COCODetection] image_path = ./data/coco/images/
    #=====> [COCODetection] info_file = ./data/coco/annotations/instances_train2017.json
