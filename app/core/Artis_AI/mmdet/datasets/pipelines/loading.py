...
class LoadDepthFromFile:
    ...
    def __call__(self, results):
        ...
        filename = filename.replace('_Color.jpg', '_Depth.txt')
        depth_file = open(filename, 'r')
    ...
    def __call__(self, results):
        ...
        with open(depth_filename, 'rb') as bin_depth_file:
            ...
            if category_depth_header == 'Version':
                ...
                # Depth 헤더가 잘못된 경우 빈 depth 채널 생성
                resize_depth = np.zeros((image_resolution_rgb[0], image_resolution_rgb[1], 1), dtype=img.dtype)

        if self.to_float32:
            resize_depth = resize_depth.astype(np.float32)

        result_img = np.concatenate((img, resize_depth), axis=2)

        results['filename'] = filename
        results['ori_filename'] = results['img_info']['filename']
        results['img'] = result_img
        results['img_shape'] = result_img.shape
        results['ori_shape'] = result_img.shape
        results['img_fields'] = ['img']
