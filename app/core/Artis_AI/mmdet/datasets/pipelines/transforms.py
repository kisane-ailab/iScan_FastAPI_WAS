...
class Resize_Depth:
    ...
    def _rescale(self, image, scale: Tuple[int, int]) -> Tuple[torch.Tensor, float]:
        """
        Resizes the input image to match the scale and calculates the scale factor.

        Args:
            image: The input image.
            scale (Tuple[int, int]): The target scale.

        Returns:
            Tuple[torch.Tensor, float]: A tuple containing the resized image tensor and the scale factor.

        """
        # PerfectMartini
        # TODO: 16bit converting
        if isinstance(image, torch.Tensor):
            image = image.to(torch.float32)
        else:
            image = F.to_tensor(image).to(torch.float32) * 255
    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.keep_ratio:
                img, scale_factor = self._rescale(results[key], results['scale'])
                # the w_scale and h_scale has minor difference
                new_w, new_h = img.shape[1:3]
                w, h = results[key].shape[1:3]
                w_scale = new_w / w
                h_scale = new_h / h
            else:
                '''
                ...
                '''
                c, h, w = results[key].shape[0], results[key].shape[1], results[key].shape[2]
                if isinstance(results[key], torch.Tensor):
                    img = results[key].cpu()
                np.resize(img, (results['scale'][1], results['scale'][0]))
                new_h, new_w = img.shape[1], img.shape[2]
                w_scale = new_w / w
                h_scale = new_h / h
            results[key] = img
            scale_factor = torch.tensor([w_scale, h_scale, w_scale, h_scale])
            results['img_shape'] = (img.shape[1], img.shape[2], img.shape[0])
            # in case that there is no padding
            results['pad_shape'] = (img.shape[1], img.shape[2], img.shape[0])
...
class Pad_Depth:
    ...
    def _pad_to_multiple(self, image: torch.Tensor, divisor: int, pad_val: Optional[int] = 0) -> torch.Tensor:
        """
        Pad an image to ensure each edge is a multiple of the given divisor.

        Args:
            image (torch.Tensor): Image to be padded.
            divisor (int): Padded image edges will be multiple of this divisor.
            pad_val (int, optional): Value to use for padding. Defaults to 0.
        ...
        w_pad = int(math.ceil(w / divisor) * divisor)
        h_pad = int(math.ceil(h / divisor) * divisor)
        w_pad = max(w_pad - w, 0)
        h_pad = max(h_pad - h, 0)

        return F.pad(image, (0, 0, w_pad, h_pad), fill=pad_val, padding_mode='constant')
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.pad_to_square:
                max_size = max(results[key].shape[1:3])
            ...
            if self.size is not None:
                ...
                padded_img = self._pad_to_multiple(
                    results[key], self.size_divisor, pad_val=pad_val)
            results[key] = padded_img
        results['pad_shape'] = (padded_img.data.shape[1], padded_img.data.shape[2], padded_img.data.shape[0])
...
class Normalize_Depth:
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            results[key] = normalize(results[key])
