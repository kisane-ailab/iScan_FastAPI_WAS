...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def _bbox_post_process(self,
        ...
        if with_nms:
            ...
            # MLSYS: Added support for 16-bit operation
            if mlvl_bboxes.dtype == torch.float32:
                det_bboxes, keep_idxs = batched_nms(mlvl_bboxes, mlvl_scores,
                                                    mlvl_labels, cfg.nms)
            elif mlvl_bboxes.dtype == torch.float16:
                det_bboxes, keep_idxs = batched_nms(mlvl_bboxes.to(dtype=torch.float32), 
                                        mlvl_scores.to(dtype=torch.float32), mlvl_labels, cfg.nms)
                det_bboxes = det_bboxes.to(dtype=torch.float16)
            else:
                raise TypeError(f"Unsupported data type: {mlvl_bboxes.dtype}")
